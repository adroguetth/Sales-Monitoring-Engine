...
===========================================================
Automated generator for daily retail sales data with realistic demand simulation.

Features:
- Generates daily sales fact table for 5 stores and 25 products.
- Applies base demand with seasonal, holiday, payday, and weekend multipliers.
- Simulates promotions with type selection, decay over time, and post-promotion penalties.
- Stockout simulation based on inventory levels and optional AI risk prediction.
- Handles store closures (total or early closure) and location-specific factors.
- Outputs to SQLite database with rich analytical columns (day_of_week, factors, etc.).
...
# Structure: product_id: (name_en, category_en, base_demand, promo_sensitivity,
...
TOTAL_CLOSURE_HOLIDAYS = [
    ...
    ("09-18", "Fiestas Patrias"),  # First Governing Body of 1810, National day
    ("09-19", "Army Day"),  # Day of the Glories of the Army
    ("12-25", "Christmas"),
]
...
def get_payday_multiplier(date: datetime, store_id: int) -> float:
    """
    Calculate payday effect: first 5 days and last 3 days of month.
    Uses calendar.monthrange to safely handle December.
    """
    day = date.day
...
def get_seasonal_factor(date: datetime, product_id: int, store_id: int) -> float:
    """
    Apply seasonal multipliers: summer (Dec-Feb) and winter (Jun-Aug).
...
def get_holiday_pre_factor(date: datetime, product_id: int, year: int) -> float:
    """
    Calculate pre‑holiday demand boost that increases as the holiday approaches.
...
def get_holiday_post_factor(date: datetime, product_id: int, year: int) -> float:
    """
    Calculate post‑holiday demand drop that recovers linearly over days_post.
...
def get_promotion(product_id: int, date: datetime, store_id: int) -> Tuple[bool, Optional[str], float, Optional[str]]:
    ...
    # Check pre‑holiday period for increased promotion probability
    year = date.year
...
# ============================================================================
# DEEPSEEK INTELLIGENCE ENGINE (Optional AI)
# ============================================================================

class DeepSeekIntelligence:
    """AI‑powered pattern recognition for stockout prediction."""
    ...
    def predict_stockout_risk(self, product_id: int, store_id: int,
                              ...
                              demand: int) -> float:
        """
        Predict probability of stockout based on historical patterns.
...
class SalesDataGenerator:
    """Daily sales fact table generator with inventory and promotion simulation."""

    FRESH_PRODUCTS = [11, 12, 13, 14, 15, 16]  # Daily restock items
    ...
    def _initialize_stocks(self) -> Dict:
        """Initialise stock levels for all stores and products."""
        stocks = {}
    ...
    def create_fact_table(self):
        """Create the sales fact table and indexes if they don't exist."""
    ...
    def _get_post_promo_penalty(self, product_id: int, store_id: int, date: datetime) -> float:
        """
        Apply a demand penalty for 2‑3 days after a promotion ends.
        Returns multiplier between 0.85 and 1.0.
        """
        cursor = self.conn.cursor()
        cursor.execute("""
        ...
        if count > 0:
            ...
            if row and row[0]:
                ...
                days_after = 3
            # Linear recovery from 0.85 to 1.0 over 2 days
    ...
    def _calculate_base_demand(self, product_id: int, date: datetime, store_id: int) -> int:
        """
        Calculate base demand using all factors except promotions.
        Returns integer >= 1.
        """
        ...
        # Month factor: first week high, last week low
        if date.day <= 7:
            month_factor = 1.25
        elif date.day >= 22:
            month_factor = 0.85
    ...
    def generate_daily_sales(self, target_date: datetime) -> pd.DataFrame:
        """
        Generate complete daily sales fact table for all stores and products.
        ...
        if is_early_close:
            logging.info(f"{target_date.strftime('%Y-%m-%d')} - Early closure (factor {operation_factor})")
        ...
        for store_id in self.stores:
            for product_id in self.products:
                ...
                # Apply stockout logic (never sell more than available)
...
def main():
    ...
    Generates daily sales fact table for a given date (default: yesterday).
    Output database path defaults to data/sales/sales_YYYY-MM-DD.db.
    """
    parser = argparse.ArgumentParser(description='Daily Sales Fact Table Generator')
    parser.add_argument('--date', type=str, help='Target date (YYYY-MM-DD). Defaults to yesterday')
    parser.add_argument('--db', type=str, default=None,
                        help='Output database path. Default: data/sales/sales_YYYY-MM-DD.db')
    ...
    if args.date:
        ...
        target_date = get_yesterday()
        logging.info(f"No date specified, using yesterday: {target_date.strftime('%Y-%m-%d')}")
    ...
    if success:
        logging.info(f"Sales data generated -> {db_path}")
    else:
        logging.error(f"Generation failed for {target_date.strftime('%Y-%m-%d')}")
