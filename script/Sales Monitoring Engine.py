...
# ============================================================================
# PRODUCT CONFIGURATION (25 products with fixed codes for future relationships)
...
# Structure: store_id: (name, zone, ticket_multiplier, promo_sensitivity, location_factors)
# ============================================================================
...
STORES = {
    1: {
        ...
        "location_factors": {11: 1.3, 21: 1.4, 22: 1.3},  # Pan, Vinos, Cerveza
...
# ============================================================================
# HOLIDAY CONFIGURATION (with dynamic Easter calculation)
# ============================================================================
...
# Pre-holiday demand effects (progressive multipliers)
PRE_HOLIDAY_EFFECTS = {
    "new_year": {
...
def get_good_friday(year: int) -> datetime:
    ...
    Technical: Implements the anonymous Gregorian algorithm for Easter calculation.
...
def get_payday_multiplier(date: datetime, store_id: int) -> float:
    ...
    """
    day = date.day
    last_day = (date.replace(month=date.month + 1, day=1) - timedelta(days=1)).day if date.month < 12 else 31
...
def get_seasonal_factor(date: datetime, product_id: int, store_id: int) -> float:
    ...
    if month in [12, 1, 2]:
        ...
        if product_id in [14, 15]:  # Fruits, Vegetables
            return np.random.uniform(1.15, 1.35)
...
def get_promotion(product_id: int, date: datetime, store_id: int) -> Tuple[bool, Optional[str], float, Optional[str]]:
    ...
    if random() < prob:
        ...
        if is_pre_holiday:
            promo_type = choice(["2x1", "discount_20", "discount_30"], p=[0.4, 0.4, 0.2])
            multiplier = PROMOTION_TYPES[promo_type]["base_multiplier"] * uniform(1.0, 1.2)
        ...
        if is_weekend and category in ["bebidas", "alcohol"]:
            promo_type = choice(["2x1", "discount_20"], p=[0.6, 0.4])
            multiplier = PROMOTION_TYPES[promo_type]["base_multiplier"] * uniform(0.9, 1.1)
            promo_value = None if "discount" not in promo_type else promo_type.split("_")[1]
            return True, promo_type, multiplier, promo_value
        
        # Standard promotion selection
        if category == "frescos":
            promo_type = choice(["discount_10", "discount_20"], p=[0.6, 0.4])
        elif product_id in [21, 22]:  # Wines, Beer
            promo_type = choice(["2x1", "bogo_2+1", "discount_20"], p=[0.4, 0.3, 0.3])
        elif promo_sens > 0.7:
            promo_type = choice(["2x1", "discount_30", "bogo_1+1"], p=[0.4, 0.3, 0.3])
        else:
            promo_type = choice(["2x1", "3x2", "discount_15"], p=[0.4, 0.3, 0.3])
...
class DeepSeekIntelligence:
    ...
    def predict_stockout_risk(self, product_id: int, store_id: int, 
        ...
        """
        is_weekend = date.weekday() >= 5
        is_holiday_week = date.day <= 7 or date.day >= 22
...
class SalesDataGenerator:
    ...
    def _initialize_stocks(self) -> Dict:
        ...
        for store_id in self.stores:
            for product_id, (_, _, _, _, _, max_stock) in PRODUCTS.items():
                stocks[(store_id, product_id)] = int(max_stock * np.random.uniform(0.7, 1.0))
        return stocks
    ...
    def _apply_promotion_decay(self, promo_key: Tuple, current_date: datetime) -> Tuple[bool, float]:
        """
    ...
    def generate_daily_sales(self, target_date: datetime) -> pd.DataFrame:
        ...
        for store_id in self.stores:
            for product_id, product_info in PRODUCTS.items():
                ...
                promo_key = (store_id, product_id, target_date.strftime("%Y-%m"))
                ...
                # Apply stockout logic
                if current_stock < theoretical_demand * 0.15 or np.random.random() < stockout_risk:
